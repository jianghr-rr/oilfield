...
export const TimeOrTimesType = {
  validator(value) {
    if (Array.isArray(value)) {
      ...
      return (
        typeof value === "string" || isNil(value) || moment.isMoment(value)
      );
    }
  }
};
  ...
  if (Array.isArray(value)) {
    return value.map(val =>
      typeof val === "string" && val
        ? interopDefault(moment)(val, valueFormat)
        : val || null
    );
  } else {
...
export const momentToString = (value, valueFormat) => {
  if (Array.isArray(value)) {
    return value.map(val =>
      interopDefault(moment).isMoment(val) ? val.format(valueFormat) : val
    );
  } else {
    return interopDefault(moment).isMoment(value)
