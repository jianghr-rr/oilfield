  ...
  <div class="search-area" ref="root">
    ...
    <div class="search-item" :key="index" v-for="(col, index) in searchCols">
      <div v-if="col.dataType === 'boolean'" :class="['title', {active: col.search.value !== undefined}]">
        ...
        <o-switch @change="onSwitchChange(col)" class="switch" v-model="col.search.value" size="small"
        ...
        />
        <o-icon v-if="col.search.value !== undefined" class="close" @click="e => onCloseClick(e, col)" type="close-circle" theme="filled" />
      </div>
      <div v-else-if="col.dataType === 'time'" :class="['title', {active: col.search.value}]">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
        <o-time-picker :format="col.search.format" v-model="col.search.value" placeholder="选择时间" @change="(time, timeStr) => onCalendarChange(time, timeStr, col)" @openChange="open => onCalendarOpenChange(open, col)" class="time-picker" size="small" :get-popup-container="() => $refs.root"/>
      </div>
      <div v-else-if="col.dataType === 'date'" :class="['title', {active: col.search.value}]">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
        <o-date-picker :format="col.search.format" v-model="col.search.value" @change="onDateChange(col)" class="date-picker" size="small" :getCalendarContainer="() => $refs.root"/>
      </div>
      <div v-else-if="col.dataType === 'datetime'" class="title datetime active">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
        <o-date-picker :format="col.search.format" v-model="col.search.value" @change="(date, dateStr) => onCalendarChange(date, dateStr, col)" @openChange="open => onCalendarOpenChange(open, col)" class="datetime-picker" size="small" show-time :getCalendarContainer="() => $refs.root"/>
      </div>
      <div v-else-if="col.dataType === 'select'" :class="['title', {active: col.search.value !== undefined}]">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
        <o-select :allowClear="true" :options="col.search.selectOptions" v-model="col.search.value" placeholder="请选择..." @change="onSelectChange(col)" class="select" slot="content" size="small" :get-popup-container="() => $refs.selectRoot">
        </o-select>
      </div>
      <div v-else :class="['title', {active: col.search.value}]">
        <o-popover @visibleChange="onVisibleChange(col, index)" v-model="col.search.visible" placement="bottom" :trigger="['click']" :get-popup-container="() => $refs.root">
          <template v-if="col.title">
            {{col.title}}
          </template>
          <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
          ...
          <o-icon v-if="!col.search.value" class="icon-down" type="down"/>
          <div class="operations" slot="content">
            <o-button @click="onCancel(col)" class="btn" size="small" type="link">取消</o-button>
            <o-button @click="onConfirm(col)" class="btn" size="small" type="primary">确认</o-button>
          </div>
          <div class="search-overlay" slot="title">
            <o-input :id="`${searchIdPrefix}${index}`" :allow-clear="true" @keyup.esc="onCancel(col)" @keyup.enter="onConfirm(col)" v-model="col.search.value" size="small" />
          </div>
        </o-popover>
        <o-icon v-if="col.search.value" @click="e => onCloseClick(e, col)" class="close" type="close-circle" theme="filled"/>
      </div>
    </div>
  </div>
</template>
