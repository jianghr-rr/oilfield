<template>
  <div class="action-columns" ref="root">
      <o-popover v-model="visible" placement="bottomRight" trigger="click" :get-popup-container="() => $refs.root">
        <div slot="title">
          <o-checkbox :indeterminate="indeterminate" :checked="checkAll" @change="onCheckAllChange" class="check-all" />列展示
          <o-button @click="resetColumns" style="float: right" type="link" size="small">重置</o-button>
        </div>
        <o-list style="width: 100%" size="small" :key="i" v-for="(col, i) in columns" slot="content">
          <o-list-item>
            <o-checkbox v-model="col.visible" @change="e => onCheckChange(e, col)"/>
            <template v-if="col.title">
              {{col.title}}:
            </template>
            <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title"></slot>
            <template slot="actions">
              <o-tooltip title="固定在列头" :mouseEnterDelay="0.5" :get-popup-container="() => $refs.root">
                <o-icon :class="['left', {active: col.fixed === 'left'}]" @click="fixColumn('left', col)" type="vertical-align-top" />
              </o-tooltip>
              <o-tooltip title="固定在列尾" :mouseEnterDelay="0.5" :get-popup-container="() => $refs.root">
                <o-icon :class="['right', {active: col.fixed === 'right'}]" @click="fixColumn('right', col)" type="vertical-align-bottom" />
              </o-tooltip>
              <o-tooltip title="添加搜索" :mouseEnterDelay="0.5" :get-popup-container="() => $refs.root">
                <o-icon :class="{active: col.searchAble}" @click="setSearch(col)" type="search" />
              </o-tooltip>
            </template>
          </o-list-item>
        </o-list>
        <o-icon class="action" type="setting" />
      </o-popover>
  </div>
</template>
