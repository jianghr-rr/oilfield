...
class ClearableLabeledInput extends React.Component<ClearableInputProps> {
    ...
    renderSuffix(prefixCls: string) {
        const {suffix, allowClear, limitLength, disabled, value} = this.props;
        if (suffix || allowClear || limitLength) {
            return (
                <span className={`${prefixCls}-suffix`}>
                    {this.renderClearIcon(prefixCls)}
                    {suffix}
                    {
                        !disabled && limitLength && <React.Fragment>
                            <span className={classNames({
                                [`${prefixCls}-suffix-warning`]: value && value.length > limitLength
