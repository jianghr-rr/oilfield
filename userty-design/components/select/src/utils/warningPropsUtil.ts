...
function warningProps(props: SelectProps) {
    ...
    noteOnce(
        ...
        'Customize `getInputElement` should customize clear and '
        + 'placeholder logic instead of configuring `allowClear` and `placeholder`.',
    );
    ...
    noteOnce(
        ...
        + ' You can set `autoFocus` if needed.',
    );
    ...
    if (value !== undefined && value !== null) {
        ...
        || values.every(val => typeof val === 'object' && ('key' in val || 'value' in val)),
            '`value` should in shape of `{ value: string | number, label?: ReactNode }` '
    ...
    if (children) {
        ...
        toNodeArray(children).some((node: React.ReactNode) => {
            ...
            const {type} = node as {
                type: {
                    isSelectOption?: boolean; isSelectOptGroup?: boolean
                }
            };
        ...
        if (invalidateChildType) {
            warning(
                false,
                `\`children\` should be \`Select.Option\` or \`Select.OptGroup\` '
                + 'instead of \`${invalidateChildType.displayName
