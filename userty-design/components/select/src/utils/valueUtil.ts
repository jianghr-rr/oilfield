...
export function flattenOptions(options: SelectOptionsType): FlattenOptionData[] {
    ...
    function dig(list: SelectOptionsType, isGroupOption: boolean, groupKey?: string | number, groupLabel?: ReactNode) {
        list.forEach(data => {
            if (isGroupOption || !('options' in data)) {
                ...
                dig(data.options, true, getKey(data, flattenList.length), data.label);
            }
        });
    }
    ...
    if (!('props' in newOption)) {
        Object.defineProperty(newOption, 'props', {
            get() {
                warning(
                    false,
                    'Return type is option instead of Option instance. '
                    + 'Please read value directly instead of reading from `props`.',
                );
                return newOption;
            }
        });
...
export function findValueOption(
    ...
    options.forEach(flattenItem => {
        if (!flattenItem.group) {
            ...
            const {groupOption, groupLabel} = flattenItem;
            // Check if match
            optionMap.set(data.value, {...data, groupOption, groupLabel});
        }
    });
...
export const getLabeledValue: GetLabeledValue<FlattenOptionData[]> = (
    ...
    if (prevValItem && typeof prevValItem === 'object' && 'label' in prevValItem) {
        result.label = item.groupOption ? `${item.groupLabel}/${prevValItem.label}` : prevValItem.label;
      ...
      && prevValItem.label.trim() !== item[optionLabelProp].trim()
        ...
        result.label = item.groupOption?  `${item.groupLabel}/${item[optionLabelProp]}` : item[optionLabelProp];
    }
    else {
        result.label = item.groupOption ? `${item.groupLabel}/${value}` : value;
