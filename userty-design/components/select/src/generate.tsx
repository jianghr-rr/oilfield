...
export interface GenerateConfig<OptionsType extends object[]> {
    ...
    omitDOMProps?: (props: object) => {[index: string]: any};
}
    ...
    >(
        () =>
            toInnerValue(mergedValue, {
                labelInValue: mergedLabelInValue as boolean,
       ...
       */
            if (mode === 'tags' && fillOptionsWithMissingValue) {
                newOptions = fillOptionsWithMissingValue(
                    newOptions,
                    mergedValue,
                    mergedOptionLabelProp as string,
                    labelInValue as boolean
                );
            }
      ...
      = (mode === 'combobox' && getInputElement && getInputElement()) || null;
        ...
        // ===================== Icons =====================
        const {suffixIcon: inputIcon} = getIcons({
            suffixIcon: suffixIconPre,
            clearIcon,
            menuItemSelectedIcon,
            removeIcon,
            loading,
        ...
        = triggerRef.current && triggerRef.current.getPopupElement();
            ...
            if (popupElement && popupElement.contains(target as HTMLElement)) {
                const timeoutId = setTimeout(() => {
                    const index = activeTimeoutIds.indexOf(timeoutId as any);
                    if (index !== -1) {
                ...
                activeTimeoutIds.push(timeoutId as any);
            }
      ...
      = showArrow !== undefined ? showArrow : loading || mode !== 'combobox';
