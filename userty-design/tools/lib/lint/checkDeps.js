...
function getModules(filePath, globConfig, debugAST) {
    ...
    // Since import is always on the top in acud,
...
module.exports = function (done) {
    ...
    try {
        ...
        = componentModules[componentRoot] || []);
            ...
            relativeDepsModuleList.forEach(module => {
                if (componentModuleList.some(m => m.name === module.name)) {
                    return;
                }
    ...
    if (!returnCode) {
        ...
        console.log(
            chalk.cyan(
                'Style dependencies check failed. '
                    + 'You can use `--debug` with `--id=xxx` or `--file=xxx` to check detail.'
            )
        );
    }
